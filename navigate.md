...
Each PowerBI screen shows a graph with selectors above it that allow you to drill down inside the data, and shows summary results on the right side. 
...
- **Commodity**: select Electricity, Hydrogen or Methane or all, depending on the graph:
...
- **Date**: Zoom in the calendar year by moving the circles:
...
This menu is diplayed when hoovering over the graphics.
...
In full screen mode, the analysis can also be selected using a dropdown menu:
...
- **Energy Balances**: for the chosen commodity (electricity, methane, hydrogen), the quantities above the axis enter the energy system – production, imports, conversion (in), storage (in) –, while the quantities below the axis exit the energy system – consumption, exports, conversion (out), storage (out); quantities are in TWh (Low Heating Value for methane and hydrogen);
- **Capacities**: installed capacities (existing + new) for every technology, sorted in decreasing order; quantities are in GW of output capacity, or GWh for storage capacity; [separate graph in GW and in GWh???]
...
- **Cost Overview**: ...
- **Energy Storage**: quantity of commodity over time for each storage technology; quantities are in GWh for energy commodities or kt for CO<sub>2</sub>;
- **Marginal Costs**: for each commodity and each hour, the marginal cost is calculated – this should reflect the market value; cost is in €/MWh; the CO<sub>2</sub> value in €/tCO<sub>2</sub> reflects the abatement cost of CO<sub>2</sub>.